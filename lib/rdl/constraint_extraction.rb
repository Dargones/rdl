...
module RDL::Typecheck
  ...
  def self.extract_var_sol(var, category)
    ...
    if category == :arg
      non_vartype_ubounds = var.ubounds.map { |b| b.bound_type}.reject { |t| t.instance_of?(RDL::Type::VarType) }
      ...
      non_vartype_lbounds = var.lbounds.map { |b| b.bound_type}.reject { |t| t.instance_of?(RDL::Type::VarType) }
    ...
    #return sol
      ...
      if var.lbounds.empty? || (var.lbounds.size == 1 && var.lbounds[0].bound_type == RDL::Globals.types[:bot])
    ...
    if  sol.is_a?(RDL::Type::UnionType) || (sol == RDL::Globals.types[:bot]) || (sol == RDL::Globals.types[:top]) || (sol == RDL::Globals.types[:nil]) || sol.is_a?(RDL::Type::StructuralType) || sol.is_a?(RDL::Type::IntersectionType) || (sol == RDL::Globals.types[:object])
      ...
      RDL::Heuristic.rules.each { |name, rule|
        ...
        begin
          if typ
            typ = typ.canonical
            var.add_and_propagate_upper_bound(typ, [], new_cons)
            var.add_and_propagate_lower_bound(typ, [], new_cons)
=begin
            new_cons.each { |var, bounds|
              bounds.each { |u_or_l, t, _|
                ...
                var.ubounds.each { |b| puts b.bound_type }
              }
            }
=end
    ...
    begin
      ...
      sol = sol.canonical
      var.add_and_propagate_upper_bound(sol, [], new_cons)
      var.add_and_propagate_lower_bound(sol, [], new_cons)
=begin
      new_cons.each { |var, bounds|
        bounds.each { |u_or_l, t, _|
...
=end
  ...
  def self.extract_meth_sol(tmeth)
    ...
    if tmeth.block && !tmeth.block.ubounds.empty?
      non_vartype_ubounds = tmeth.block.ubounds.map { |b| b.bound_type.canonical }.reject { |t| t.is_a?(RDL::Type::VarType) }
